...
namespace {
    double distanceSquared(std::pair<int,int> a, std::pair<int,int> b) {
    ...
    bool insideHalfPlane(std::pair<double,double> p, double a, double b, double c) {
        const double eps = 1e-9;
        return a * p.first + b * p.second <= c + eps;
    }

    std::pair<double,double> lineIntersection(
    ...
    std::vector<std::pair<double,double>> clipPolygon(
        ...
        for (size_t i = 0; i < polygon.size(); i++) {
            ...
            bool currentInside = insideHalfPlane(current, a, b, c);
            bool nextInside    = insideHalfPlane(next,    a, b, c);

            if (currentInside && nextInside) {
                result.push_back(next);
            } else if (currentInside && !nextInside) {
                result.push_back(lineIntersection(current, next, a, b, c));
            } else if (!currentInside && nextInside) {
                result.push_back(lineIntersection(current, next, a, b, c));
                result.push_back(next);
            }
...
int GeoDistricts::nearestNeighbor(
    ...
    for (int i = 0; i < static_cast<int>(centrals.size()); i++) {
        ...
        double currentDistance = distanceSquared(centrals[i], query);
...
GeoDistricts::voronoiPolygons(const std::vector<std::pair<int,int>>& coords) {
    ...
    if (coords.empty()) return regions;
    ...
    double left   = minX - margin, right = maxX + margin;
    double bottom = minY - margin, top   = maxY + margin;
...
std::string GeoDistricts::formatResult(
    ...
    if (coords.empty() || centrals.empty()) return oss.str();
    ...
    for (int i = 0; i < static_cast<int>(coords.size()); i++) {
        ...
        if (i + 1 < static_cast<int>(coords.size())) oss << "\n";
    }
    return oss.str();
}